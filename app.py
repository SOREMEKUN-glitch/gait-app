...
def load_csv(source):
    """Auto-detect separator and load CSV with correct column names."""
    for sep in ['\t', ',', ';']:
        try:
            tmp = pd.read_csv(source, sep=sep)
            if tmp.shape[1] == 5:
                tmp.columns = ['time', 'accel_x', 'accel_y', 'accel_z', 'accel_abs']
                return tmp
        except Exception:
            pass
    return None
...
if use_sample:
    try:
        ...
        if df is not None:
            ...
            st.error("Could not read Raw_data.csv. Check the file has 5 columns.")
    ...
    if df is not None:
        ...
        st.error("Could not read the file. Check it has 5 columns: Time, X, Y, Z, Absolute.")
